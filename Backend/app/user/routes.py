...
def visited():
    userid = request.json.get('userId', None)
    personId = request.json.get('personId', None)
    print({"userid": userid, "personId": personId})
    if userid != None and personId != None and userid != personId:
        ...
        if count_entry > 1:
            entry = delete(browsingHistory).where(browsingHistory.c.viewer_id == userid, browsingHistory.c.viewed_id == personId).first()
            db.session.execute(entry)
    ...
    if category == "film":
        if is_main == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": film.id, "title": film.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
        else:
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": director.id, "title": director.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
    elif category == "book":
        if is_main == "true":
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": book.id, "title": book.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
        else:
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": author.id, "title": author.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
    elif category == "game":
        if is_main == "true":
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": game.id, "title": game.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
        else:
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": studio.id, "title": studio.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
    elif category == "music":
        if is_main == "true":
            if is_high == "true":
            ...
            for entry in top_rating:
                ...
                result_temp = {"id": song.id, "title": song.title, "rating": entry.rating, "review": entry.review}
                result["Items"] = result.setdefault("Items", []) + [result_temp]
        else:
            if is_high == "true":
...
@user.route("/different/", methods = ['GET'])
@jwt_required()
